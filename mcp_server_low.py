...
from utils.mcp_helpers import get_project_folder, is_relative_path, \
  to_text_context
...
async def handle_tool_call(
  ...
  try:
    ...
    logger.error("Error handling tool call '%s': %s", name, arguments,
                 exc_info=True)
    response = json.dumps(
...
async def execute_sql_statement(dbname: str, query: str,
    read_only: bool) -> str:
...
async def web_search(query: str) -> str:
  ...
  try:
    ...
    logger.error("An unexpected error occurred in query_web: %s", e,
                 exc_info=True)
    return json.dumps({"error": f"Unexpected server error: {str(e)}"})
